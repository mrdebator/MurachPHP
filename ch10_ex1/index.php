...
switch ($action) {
    ...
    case 'process_data':
        ...
        try{
          ...
          $message = 'Both dates must be in a valid format. Please try again!';

        }
        // make sure the due date is after the invoice date
        if($due_date_o < $invoice_date_o){
          $message = 'The due date must be after the invoice date. Please try again!';
        ...
        if($due_date_o < $current_date_o){
          $due_date_message = $time_span->format('The invoice is %y years, %m months, and %d days overdue!');
        }
        else{
          $due_date_message = $time_span->format('The invoice is due in %y years, %m months, and %d days.');
        }
        break;
}
